...
export async function updateClaveEmail(req, res) {
    ...
    try {
        ...
        return res.status(500).json({
            ...
            data: {}
=======
  const { correo, bloqueado, motivobloqueoid } = req.body;
  //console.log(usuarioid);
  try {
    const found = await Usuario.findAll({
      attributes: ["usuarioid"],
...
export async function updateClaveEmail(req, res) {
  const { correo, passwordencrip, password } = req.body;
  console.log("correo: ", correo);
  console.log("passwordencrip: ", passwordencrip);
  console.log("password: ", password);
  try {
    ...
    var mailOptions = {
      ...
      subject: "Servicio de recuperacion de contrasena",
      html:
        "La contrasena para el usuario con correo: <b>" +
        correo +
        "</b> ha sido modificada por <b>" +
        password +
        "</b>",
    };
    mail.sendMail(mailOptions, function (error, info) {
...
export async function insertLineaUsuario(req, res) {
  const { usuarioid, lineaid } = req.body;
  try {
...
async function getRolmenus(req) {
  ...
  try {
    let entidades = await Rolmenusubmenu.sequelize.query(
      "SELECT rolid, nombre, submenuid, menuid from public.fn_get_rolsubmenu_rolid(" +
        id +
        ")",
      {
        type: Rolmenusubmenu.sequelize.QueryTypes.SELECT,
...
>>>>>>> b66d3f4c019b7bf3c724332411f7314b86b05b4f
    ...
    var mailOptions = {
      ...
      html:
        "La contrase�a para el usuario con usuario: <b>" +
        usuario +
        "</b> ha sido creada por <b>" +
        password +
        "</b>",
    };
    mail.sendMail(mailOptions, function (error, info) {
